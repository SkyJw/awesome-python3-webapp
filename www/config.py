...
class Dict(dict):#这个类应该算是很常见了，就是把dict类加工一下，使得新的Dict类创建的实例可以用x.y的方式来取值和赋值
    ...
    '''
    def __init__(self, names=(), values=(), **kw):#name values 默认参数为一个空的元组
        super(Dict, self).__init__(**kw)#先调用父类的初始化方法存储键值对
        for k, v in zip(names, values):#添加形如dict(('name', 'age'), ('Sky', '24'))的方法来创建字典
            self[k] = v
    ...
    for k, v in defaults.items():
        if k in override:
            if isinstance(v, dict):
                r[k] = merge(v, override[k]) #递归调用，合并子字典
            else:
                r[k] = override[k]
        else:
            r[k] = v
